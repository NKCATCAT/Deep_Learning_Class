...
def load_data_fashion_mnist(batch_size, resize=None):
    # Use the calculated mean and standard deviation for normalization
    mean = [0.2860]
    std = [0.3205]
    ...
    # Add random flip transformations only to the training set
    train_trans = trans + [transforms.RandomVerticalFlip(), transforms.RandomHorizontalFlip()]

    # Compose the transformations
    train_transform = transforms.Compose(train_trans)
    test_transform = transforms.Compose(trans)  # No random flips for test set

    # Load the datasets with the corresponding transformations
    mnist_train = datasets.FashionMNIST(root="./data", train=True, transform=train_transform, download=True)
    mnist_test = datasets.FashionMNIST(root="./data", train=False, transform=test_transform, download=True)
...
def train_test_save_fig(net, train_iter, test_iter, num_epochs, lr, device, save_dir="results"):
    ...
    plt.savefig(os.path.join(save_dir, "training_plot_resnet34.png"), bbox_inches="tight")

# Network definition for ResNet-34
...
lr, num_epochs, batch_size = 0.001, 50, 512
train_iter, test_iter = load_data_fashion_mnist(batch_size, resize=96)

device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
train_test_save_fig(resnet34, train_iter, test_iter, num_epochs, lr, device)